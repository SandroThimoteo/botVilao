...
//   }
  ...
  // Procurar membro pelo passaporte
  ...
  try {
    ...
    const registryEntry = Object.values(registrations).find(r => r.passaporte === passaporte);
  ...
  try {
    ...
    // Se punição contém "curso" ou similar, remover cursos básicos
    if (punicao.toLowerCase().includes('curso')) {
      ...
      if (basicCourses.length > 0) {
        await targetMember.roles.remove(basicCourses, 'Advertência - Remoção de cursos').catch(e => {
          console.error('[Advertencia] Erro ao remover cursos:', e);
        });
        punitionLog.push(`✅ Removido ${basicCourses.length} cursos básicos`);
      }
    }
    ...
    if (punicao.toLowerCase().includes('patente') || punicao.toLowerCase().includes('rebaixa')) {
      ...
      if (patenteHierarquia.length > 0) {
        ...
        if (memberPatentes.length > 0) {
          await targetMember.roles.remove(memberPatentes[0], 'Advertência - Rebaixamento de patente').catch(e => {
            console.error('[Advertencia] Erro ao remover patente:', e);
          });
    ...
    if (punicao.toLowerCase().includes('unidade')) {
      ...
      if (memberUnidade.length > 0) {
        await targetMember.roles.remove(memberUnidade[0], 'Advertência - Remoção de unidade').catch(e => {
  ...
  if (logChannel) {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🆔 Passaporte', value: passaporte, inline: false },
